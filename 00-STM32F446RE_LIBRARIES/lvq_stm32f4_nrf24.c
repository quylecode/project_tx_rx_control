...
static inline uint8_t LVQ_NRF24_RW(LVQ_NRF24_Config_t nrf24, uint8_t data) 
{
    ...
    uint8_t result;
    result = LVQ_SPI_Send(nrf24.NRF24_SPI, data);
    return result;
}
...
{
		/* CNS pin */
		LVQ_GPIO_Init(nrf24.NRF24_CSN_PORT, nrf24.NRF24_CSN_PIN, LVQ_GPIO_Mode_OUT, LVQ_GPIO_OType_PP, LVQ_GPIO_PuPd_UP, LVQ_GPIO_Speed_High);

		/* CE pin */
		LVQ_GPIO_Init(nrf24.NRF24_CE_PORT, nrf24.NRF24_CE_PIN, LVQ_GPIO_Mode_OUT, LVQ_GPIO_OType_PP, LVQ_GPIO_PuPd_UP, LVQ_GPIO_Speed_High);
...
uint8_t LVQ_NRF24_TXSearchChannel(LVQ_NRF24_Config_t nrf24)
{
		...
		while ( u16TimeOut--) 
		{
				if (LVQ_NRF24_GetStatus_RXFIFO(nrf24) != LVQ_NRF24_STATUS_RXFIFO_EMPTY) 
				{
						...
						uint8_t pipe = LVQ_NRF24_ReadPayload(nrf24, u8DataCheck, &u8SizePayLoad);
...
void LVQ_NRF24_HandleData(uint8_t* pString, uint8_t* pOutput)
{
		...
		pOutput[0] = DataFrame.Data.dataX;
		pOutput[1] = DataFrame.Data.dataY;
		pOutput[2] = DataFrame.Data.dataZ;
		pOutput[3] = DataFrame.Data.dataF;
		pOutput[4] = DataFrame.Data.dataG;
		pOutput[5] = DataFrame.Data.dataH;
